...
public class ActiveUserFragment extends Fragment implements ResponseCallBack {
    GridView gridView;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.activity_active_user_fragment, container, false);
        SharedPreferences prefs = getContext().getSharedPreferences("shared", Context.MODE_PRIVATE);
        userService = new UserService();
        id = prefs.getInt("id", 0);
        gridView = view.findViewById(R.id.activeUser);
        getUsers();
        return view;
    }

    public ActiveUserFragment(ResponseCallBack responseCallBack){
    ...
    public void getUsers() {
        userService.getActiveUsers(this);
    }
    ...
    public void onSuccess(Response response) throws IOException {
        ...
        if (!users.isEmpty()) {
            ClientsAdapter adapter = new ClientsAdapter(users, getContext(), "active",responseCallBack);
