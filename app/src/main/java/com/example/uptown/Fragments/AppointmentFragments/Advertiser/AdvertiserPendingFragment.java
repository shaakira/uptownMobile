...
import com.example.uptown.Adapters.AdvertiserPendingAdapter;
...
public class AdvertiserPendingFragment  extends Fragment implements ResponseCallBack {
    View view;
    Integer id;
    AppointmentService appointmentService;
    GridView gridView;
    LinearLayout empty;
    public AdvertiserPendingFragment() {
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.advertiser_pending_fragment, container, false);
        SharedPreferences prefs = getContext().getSharedPreferences("shared", Context.MODE_PRIVATE);
        appointmentService = new AppointmentService();
        id = prefs.getInt("id", 0);
        gridView=view.findViewById(R.id.advertiserPending);
    ...
    public void getAppointments(int advertiserId){
        appointmentService.findPendingAppointments(advertiserId,this);
    }
    ...
    public void onSuccess(Response response) throws IOException {
        ...
        if(!appointments.isEmpty()){
            AdvertiserPendingAdapter adapter=new AdvertiserPendingAdapter(appointments,getContext());
